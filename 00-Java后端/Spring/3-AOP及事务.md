...
- 体现了 Spring理念：无入侵式/无侵入式

#### 1.2 AOP中的核心概念【理解】

![image-20210730144903209](img/image-20210730144903209.png)
...
![image-20210730150138448](img/image-20210730150138448.png)
...
![image-20210730154740528](img/image-20210730154740528.png)
...
![image-20210730151347354](img/image-20210730151347354.png)
...
![image-20210730152631741](img/image-20210730152631741.png)
...
- AOP通知共分为5种类型
  ...
  - **<u>环绕通知(重点)：</u>**手动调用切入点方法并对其进行增强的通知方式。
...
1. 环绕通知方法形参必须是**ProceedingJoinPoint**，表示正在执行的连接点，使用该对象的**proceed()方法**表示对原始对象方法进行调用，返回值为原始对象方法的返回值。
...
![image-20210730164219555](img/image-20210730164219555.png)
...
> 说明：在前置通知和环绕通知中都可以获取到连接点方法的参数

- **JoinPoint**对象描述了连接点方法的运行状态，可以获取到原始方法的调用参数**getArgs()**
...
说明：在**@AfterReturning通知**和**@Around环绕通知**中都可以获取到连接点方法的返回值
...
![image-20210731193059709](img/image-20210731193059709.png)

分析：
①：在业务方法执行之前对所有的输入参数进行格式处理——trim()
②：使用处理后的参数调用原始方法——**环绕通知**中存在对原始方法的调用
...
![image-20210801190820853](img/image-20210801190820853.png)
...
1. Spring注解式事务通常添加在**业务层接口**中而不会添加到业务层实现类中，降低耦合
2. 注解式事务可以添加到**业务方法**上表示当前方法开启事务，也可以添加到**接口**上表示当前该接口所有方法开启事务

##### 【第二步】设置事务管理器(将事务管理器添加到IOC容器中)

> 说明：可以在JdbcConfig类中配置事务管理器
...
![image-20210802151553053](img/image-20210802151553053.png)

说明：对于**RuntimeException类型**异常或者**Error错误**，Spring事务**能够进行回滚**操作。但是对于**受查异常比如 IOException**，Spring事务是不进行回滚的，所以需要使用rollbackFor来设置要回滚的异常。
...
注意：AccountService接口上已经添加了 @Transactional注解
...
也就是说，被@Transactional注解的方法作为事务协调员时（它被调用），应该怎么做。

![image-20210802153216460](img/image-20210802153216460.png)

![image-20210802153014296](img/image-20210802153014296.png)
